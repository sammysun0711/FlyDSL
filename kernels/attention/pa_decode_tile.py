...
in the MiMo 8-element vectorized-5D cache layout. The FP8 path uses
    ...
    *,
    ...
    if bf16_kv:
        if arch not in BF16_KV_SUPPORTED_ARCHS:
            raise ValueError(
                "BF16 vectorized-5D PA decode requires gfx942 or gfx950; "
                f"got {arch}"
            )
    ...
    MFMA_K = BF16_MFMA_K if bf16_kv else FP8_MFMA_K
    MFMA_VALUES_PER_LANE = 4 if bf16_kv else 8
    ...
    def pa_decode_tile_kernel(
        ...
        # not divisible by 8: D192 has QCHUNK=12 and needs three 4-element
        # loads, otherwise one third of each query row remains unstaged.
        ...
        if const_expr(not per_token_kv and not bf16_kv):
            key_scale = fx.Int32(
        ...
        def _k_ops(phys, a):
            # Page ids fit in i32, but cache offsets cross i32 above 2 GiB.
            phys = fx.Int64(phys)
            ...
            for qkhe in range_constexpr(QKHE_LOOP):
                ...
                if const_expr(bf16_kv):
                    for qkr in range_constexpr(2):
                        ...
                        base = (
                            (((phys * n_kv + kv_h) * (head_dim // 8) + he8_idx) * block_size + within_page_tok)
                            * 8
                        )
                    ...
                    base = (
                        (((phys * n_kv + kv_h) * QCHUNK + he_idx) * block_size + within_page_tok)
                        * QK_CHUNK_ELEMS
                    )
        ...
        STEPS_PER_PAGE = block_size // (8 if bf16_kv else 16)
        ...
        for tt, ostate in range(part_start, part_end, 1, init=init_state):
            ...
            q_scale_vec = None
            if const_expr(M_TILES > 1 and not bf16_kv):
            ...
            if const_expr(M_TILES > 1):
                ...
                for m in range_constexpr(M_TILES):
                    ...
                    scale = scale_qk if const_expr(bf16_kv) else scale_qk * fx.Float32(q_scale_vec[m])
                ...
                scale = scale_qk if const_expr(bf16_kv) else scale_qk * _ld1(sQscale_off, lane16)
        ...
        for m in range_constexpr(M_TILES):
            ...
            if const_expr(per_token_kv or bf16_kv):
                o_scale = inv_l
            else:
                o_scale = fx.Float32(
                    arith.mulf(
...
def pa_decode_tile(
    ...
    *,
    ...
    expected_vector = 8 if bf16_kv else 16
